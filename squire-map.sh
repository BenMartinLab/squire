...
usage() {
  ...
  echo ""
  echo "Any additional parameters will be passed to squire Map"
  echo ""
  echo "Do not use --name, --read1 or --read2 parameters for squire Map as they will be set from the samplesheet."
}
...
while [ "$1" != "" ]; do
  case $1 in
    ...
    -h | --help)
      ...
      echo ""
      echo ""
      echo "Squire Map help."
      bash "${script_path}/squire.sh" Map -h
      exit 0
      ;;
    *)
      extra_parameters+=("$1")
...
bash "${script_path}/squire.sh" Map \
    --pthreads "$threads" \
    --name "$sample" \
    --read1 "$reads_1" \
    "${reads_2_parameters[@]}" \
